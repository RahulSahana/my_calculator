...
class _SpeedState extends State<Speed> {
  ...
  final List<String> _units = ["km/h", "m/s", "mph", "knot", "mach"];

  final Map<String, double> _conversionToKmh = {
    "km/h": 1.0,
    "m/s": 3.6,
    "mph": 1.60934,
    "knot": 1.852,
  ...
  String _convert(String value, String from, String to) {
    ...
    double inKmh = val * (_conversionToKmh[from] ?? 1);
    double result = inKmh / (_conversionToKmh[to] ?? 1);
  ...
  String _getLabel(String unit) {
    switch (unit) {
      case "km/h": return "Kilometers per hour";
      case "m/s": return "Meters per second";
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Expanded(child: Center(child: Text("Speed Converter", style: TextStyle(color: Colors.purpleAccent, fontSize: 22)))),
