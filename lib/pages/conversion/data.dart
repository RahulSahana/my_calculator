...
class _DataState extends State<Data> {
  ...
  final Map<String, double> _conversionToByte = {
    "B": 1.0,
    "KB": 1024.0,
    "MB": 1024.0 * 1024.0,
  ...
  String _convert(String value, String from, String to) {
    ...
    double inBytes = val * (_conversionToByte[from] ?? 1);
    double result = inBytes / (_conversionToByte[to] ?? 1);

    if (result == result.toInt()) return result.toInt().toString();
    if (result < 0.000001) return result.toStringAsExponential(2);
    return result
        .toStringAsFixed(6)
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Expanded(child: Center(child: Text("Data Converter", style: TextStyle(color: Colors.purpleAccent, fontSize: 22)))),
